...
(define-judgment-form
 ...
 [(types Γ e_1 (→ T_2 T_3)) (types Γ e_2 T_2) ------------------------- (types Γ (e_1 e_2) T_3)]
 [(types (x : T_1 Γ) e T_2) ----------------------------------- (types Γ (λ (x T_1) e) (→ T_1 T_2))]
 [--------------------- (types (x : T Γ) x T)]
 [(types Γ x_1 T_1)
  (side-condition (different? x_1 x_2))
  ------------------------------------
  (types (x_2 : T_2 Γ) x_1 T_1)]
 [(types Γ e Number) ... ----------------------- (types Γ (+ e ...) Number)]
...
(pretty-derivation (build-derivations (types (f : (→ Bool Bool) ∅) (f (if x false x)) Bool)))
